...
description: Agent skills for working with Minions Audio-tests MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-audio-tests data.
---

# Minions Audio-tests Agent Skills

Skills for agents operating on the `minions-audio-tests` toolbox.
...
```bash
# TypeScript
pnpm add @minions-audio-tests/sdk
...
The `audio-tests` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-audio-tests/sdk/schemas';
...
```python
from minions_audio_tests.schemas import custom_types
...
- This agent only writes to `minions-audio-tests` — it reads from other toolboxes but never writes to them
